...
const SidebarLayout = ({ children }) => {
    ...
    return (
        <>
            ...
            <BasicMenu />
            {/* 상단 여백 my-5 제거 */}
            <div className="bg-zinc-100 w-full flex flex-col flex px-8 gap-2 space-y-4  md:flex-row md:space-x-4 md:space-y-0">
                {/* 첫 번째와 다섯 번째 열은 isMobile이 true일 때 hidden 클래스 추가 */}
                <div className={`col-span-1 p-2 w-full max-w-[20rem]  ${isMobile ? "hidden" : ""}`}>
                    <Sidebar />
                </div>
                {/* 두 번째와 네 번째 열은 isMobile이 true일 때 hidden 클래스 추가 */}
                <main className="bg-white w-full  md:col-span-2 m-0">
                    {children}
                </main>
                {/* 세 번째와 여섯 번째 열은 isMobile이 true일 때 hidden 클래스 추가 */}
                <div className={`col-span-1 p-2 w-full max-w-[20rem] mt-20  ${isMobile ? "hidden" : ""}`}>
                    <SideCell/>
                </div>
            </div>
        </>
